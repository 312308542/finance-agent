export type TaskMonitorStatus = "running" | "paused" | "waiting" | "completed" | "failed" | "unknown";
...
const jobDescriptionMap: Record<string, { title: string; description: string }> = {
  "ashare.universe.all": {
    title: "A 股资产池刷新",
...
function normalizeTaskItem(raw: Record<string, any>, isRealtime: boolean): TaskMonitorItem {
  ...
  const jobDescription = describeSchedulerJob(jobName);
...
export function describeSchedulerJob(jobName: string | undefined): { title: string; description: string } {
