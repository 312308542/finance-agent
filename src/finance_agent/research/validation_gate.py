...
class StrategyValidationGate:
    ...
    def evaluate_history(self, result: Any) -> ValidationDecision:
        ...
        payload = _mapping(_value(result, "payload", {}))
        ...
        cross_sections = _count(metrics.get("valid_cross_sections"))
        ...
        if metrics.get("data_integrity_violations"):
            reasons.append("historical_data_integrity_violations")
        horizon_means = _mapping(metrics.get("horizon_mean_excess_returns"))
        ...
        bootstrap = _mapping(metrics.get("t10_block_bootstrap"))
        ci = bootstrap.get("ci_95") or bootstrap.get("ci95") or ()
        valid_interval = isinstance(ci, Sequence) and not isinstance(ci, (str, bytes)) and len(ci) == 2
        lower = _number(ci[0]) if valid_interval else None
        upper = _number(ci[1]) if valid_interval else None
        if lower is None or upper is None or lower <= 0 or upper < lower:
            reasons.append("t10_bootstrap_lower_bound_not_positive")
        if _count(metrics.get("positive_t10_phase_count")) < 2:
            reasons.append("fewer_than_two_positive_t10_phases")
        drawdown_gap = _number(metrics.get("drawdown_gap"))
        if drawdown_gap is None:
            reasons.append("drawdown_gap_missing_or_invalid")
        elif drawdown_gap > MAX_DRAWDOWN_GAP:
        ...
        if rank_ic_mean is None or not MINIMUM_RANK_IC <= rank_ic_mean <= 1:
        ...
        if weekly_turnover is None or not 0 <= weekly_turnover <= MAX_WEEKLY_TURNOVER:
        ...
        if unexecutable_rate is None or not 0 <= unexecutable_rate <= MAX_UNEXECUTABLE_RATE:
    ...
    def evaluate_forward(self, *, state: Any, outcomes: Any) -> ValidationDecision:
        ...
        if not evidence_id:
            reasons.append("validation_evidence_missing")
        t20_count = _count(metrics.get("t20_count"))
        metrics["t20_count"] = t20_count
        if t20_count < MINIMUM_T20_SAMPLES:
            reasons.append("t20_samples_below_60")
        rolling_excess = _number(metrics.get("rolling_excess"))
        if rolling_excess is None:
            reasons.append("rolling_excess_missing_or_invalid")
        elif rolling_excess < 0:
            reasons.append("rolling_excess_negative")
        median_excess = _number(metrics.get("median_excess"))
        if median_excess is None:
            reasons.append("median_excess_missing_or_invalid")
        elif median_excess <= 0:
            reasons.append("median_excess_not_positive")

        failure_count = _count(_value(state, "consecutive_failure_count", 0))
        hard_failure = bool(
            t20_count >= MINIMUM_T20_SAMPLES
            and rolling_excess is not None
            and median_excess is not None
            and "median_excess_not_positive" in reasons
        )
        ...
        allowed = next_state == "validated" and not reasons
    ...
    def evaluate_runtime(self, trial_state: Any, *, action: str) -> ValidationDecision:
        ...
        metrics: JsonDict = {}
        exit_actions = {
            "hold", "watch", "avoid", "reduce", "sell", "exit", "unexecutable", "add_blocked",
            "discovered", "setup_confirming", "active", "weakening", "exit_pending",
            "exited", "cooldown",
        }
        buy_actions = {"buy", "buy_candidate", "strong_buy", "buy_ready", "add"}
        if normalized_action in exit_actions:
            allowed = True
            reasons: tuple[str, ...] = ()
        elif normalized_action in buy_actions:
            forward = self.evaluate_forward(
                state=trial_state,
                outcomes=_value(trial_state, "forward_metrics", {}),
            )
            metrics = forward.metrics
            allowed = current_state == "validated" and forward.allowed
            reasons = () if allowed else tuple(dict.fromkeys((
                "strategy_disabled_new_buy_blocked" if current_state == "disabled"
                else "strategy_not_validated_new_buy_blocked",
                *forward.reason_codes,
            )))
        else:
            allowed = False
            reasons = ("strategy_state_action_blocked",)
        return ValidationDecision(
            allowed=allowed,
            current_state=current_state,
            next_state=current_state,
...
def _normalize_forward_metrics(outcomes: Any) -> JsonDict:
    if isinstance(outcomes, Mapping):
        ...
        sample_counts = _mapping(metrics.get("sample_counts"))
        medians = _mapping(metrics.get("median_excess_returns"))
    ...
    t20 = [row for row in rows if _count(_value(row, "horizon_days", 0)) == 20]
