...
AKSHARE_CAPABILITIES: tuple[AkshareCapability, ...] = (
    ...
    AkshareCapability(
        ...
        storage_targets=("raw_records",),
        factor_groups=("index", "sector"),
        description="A 股指数目录，用于自动展开沪深 300、中证 500、创业板等指数成分种子池。",
        enabled_in_mvp=True,
    ),
    AkshareCapability(
        ...
        factor_groups=("index",),
        description="中证指数成分股，用于指数候选池成员；失败时可降级到新浪/通用指数成分接口。",
        fallback_names=("index_stock_cons_sina", "index_stock_cons"),
        sample_params={"symbol": "000300"},
        enabled_in_mvp=True,
    ),
    AkshareCapability(
