...
def test_frontend_closed_loop_docs_reflect_07_completion() -> None:
    """07 方案收尾后，README 和进度文档应同步前端闭环状态。"""

    readme = read_doc("README.md")
    project_progress = read_doc("docs/项目进度跟踪表.md")
    optimization_progress = read_doc("docs/优化版本进度跟踪表.md")
    overall_plan = read_doc("docs/开发方案/00-总体规划与执行约定.md")
    frontend_plan = read_doc("docs/开发方案/07-前端用户闭环页面.md")

    assert "报告、推荐、持仓、风险、Agent 运行和 Finance Memory 页面已接真实后端只读接口" in readme
    assert "推荐页反馈会调用决策反馈 API，真实落库验收留到统一联调阶段" in readme

    assert "| 前端办公室 | 07 前端用户闭环页面主体完成 |" in project_progress
    assert "报告页、推荐页、持仓页、风险中心、Agent 运行页、Finance Memory 页均已接真实 API" in project_progress

    assert "| O8 | 前端接真实后端 | 用户闭环页面主体完成，端到端联调待补 |" in optimization_progress
    assert "| 中文报告展示 | 已完成 |" in optimization_progress
    assert "| 前端 API 层 | 已完成 |" in optimization_progress
