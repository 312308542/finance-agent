...
5. Agent 入池原因和每日继续关注原因必须进入 Finance Memory：入池写 `candidate_intake_reason` 事件和记忆；每日复核写 `daily_watch_reason` 事件和 `watchlist_daily_reason` 记忆。业务事实复用 `watchlist_item_events` 和 `assistant_memories`，图谱关系由 GraphSync 投影到配置选择的 GraphStore。
...
订单草案与执行登记闭环已实现，当前边界是“辅助决策和记账复盘”，不是交易执行系统。用户接受建议后，系统可以生成仅具文档性质的订单草案；执行登记只接受用户在外部完成交易后的手工填报，记录价格、数量、时间、费用和备注，并由系统同步当前持仓、生成到期复盘任务、比较建议价/执行价/后续表现，最后把复盘结果反写 Finance Memory。

系统仍不连接券商或交易所写接口，不提交真实订单，不撤单，不维护真实订单状态机。后续如需接入交易系统，必须新增显式权限开关、二次确认、审计记录和独立安全评审，且不能绕过本节定义的人工确认边界。
...
- `IndicatorRegistry`
- `IchimokuAdapter`：用 pandas 公式计算一目均衡五线，输出带 evidence_id 的结构化结果。
- `CorrelationAdapter`：用 pandas 对齐收益率并计算相关矩阵，供组合风险解释使用。
- `PairTradingAdapter`：用 numpy 计算轻量 OLS hedge ratio、残差价差和 z-score；严格协整检验待评估是否引入 `statsmodels`。
- `SeasonalAdapter`：用 pandas 计算月份收益画像，作为弱辅助信息，不覆盖评分和风险标记。
- `HarmonicAdapter`：保留 `pyharmonics` 适配接口；未配置确定性引擎时明确拒绝运行。
...
| Finance Memory | 投资假设、建议理由、用户采纳情况、复盘结论、风险偏好、观察池变更原因 | `user_investment_profiles`、`decision_logs`、`assistant_memories`、`review_tasks`、`asset_theses`、`memory_embeddings`、GraphStore | 作为金融建议的可审计上下文和长期业务记忆 |
...
- 图谱关系进入 GraphStore：通过配置文件在 `neo4j` 和 `apache_age` 中二选一，同一环境只启用一个后端，不自动 fallback，不双写双读。
- 默认推荐 `neo4j`，使用 Neo4j / DozerDB 承载多跳查询、风险传导、社区发现和 GraphRAG；`apache_age` 是可选部署形态。
- 观察池原因记忆最小化落库：`watchlist_item_events` 记录 `candidate_intake_reason` 和 `daily_watch_reason`，`assistant_memories` 记录对应摘要，GraphSync 再投影为 `WatchlistEvent -> Memory` 等图谱关系。
- `financial_memory_edges` 作为历史兼容表短期保留，不再作为长期知识图谱主方案。
...
- 冷启动画像由 `profile.get` 返回，不阻塞选股和顾问建议。
- 默认 `risk_appetite=balanced`、`horizon=swing`、`style_tendency={"value":0.6,"theme":0.4}`、`timing_posture=neutral`，各维度低置信度并标记 `source=default`。
- 用户确认或系统从决策/复盘中推断后，通过 `profile.upsert` 写入 `user_investment_profiles`，并同步沉淀 `assistant_memories(memory_type=investment_profile)`。
- `advice.suggest_style` 只能给风格/择时建议和证据链，不能直接修改 `AssetScore.total_score`、信号方向或风险标记。
...
### 13.3 Vibe-Trading（HKUDS，MIT）

本地路径：`D:\Code\aiAgents\Vibe-Trading`。MIT 许可证，既可作设计参考，也可在不违背本项目红线的前提下复用代码/装为依赖。它是一个成熟的个人交易 Agent，恰好实现了本项目"私人金融助手 + 选股"正在补的多个能力，是批次 6（方案 14~19）的重点借鉴对象。

可借鉴（按本项目方案对应）：

- **Agent Harness**（技能按需加载的 SKILL.md 库、5 层上下文压缩、跨会话持久记忆、自演化技能）：上层顾问 Agent 的参考 → 方案 14 总纲、方案 16。
- **Trade Journal Analyzer**（解析同花顺/东财/富途券商导出，产出持仓天数/胜率/盈亏比/回撤画像 + 处置效应/过度交易/追涨/锚定等行为偏差诊断）：从用户真实行为推断投资画像的成熟范式 → 方案 16。
- **Shadow Account**（从交易记录提取用户规则 → 回测 → 对比实际与影子策略）：画像演化与复盘学习的参考 → 方案 16、方案 19、方案 08。
- **跨会话偏好记忆**（"记住:我偏好 RSI 策略、最大回撤 10%"自动召回）：画像偏好记忆范式 → 方案 16。
- **sector-rotation / multi-factor / Alpha Zoo（456 因子）/ swarm 圆桌预设**（investment_committee 多空辩论、technical_analysis_panel、risk_committee）：双因子与圆桌角色的参考 → 方案 18、方案 04。
- 18 个只读数据工具（fund flow、dragon-tiger、northbound、sector 等）与多源 fallback 链：数据层与工具层参考 → 方案 15。

不直接复用（红线边界，重于许可证）：

- **"让 LLM 生成策略代码并自主交易"的核心哲学**与本项目"不自动交易、LLM 不覆盖确定性数据、事实只来自入库"冲突。只借鉴其 Agent/记忆/画像/技能/数据工具范式，**不引入** LLM 生成策略代码、自主下单、broker 写接口。
- 借鉴其行为画像与影子复盘思路时，本项目版本必须把画像落成**可审计实体**并驱动**确定性评分**（方案 16），而非让模型生成可执行策略。
...
- Finance Memory 采用结构化表 + 向量索引 + GraphStore 三层存储；图谱后端通过配置在 Neo4j / DozerDB 与 Apache AGE 中二选一，默认 Neo4j / DozerDB。
