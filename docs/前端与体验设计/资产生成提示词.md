...
本文用于指导用 AI 生图工具生产 `apps/agent-office` 所需资产。目标是让资产能被 Phaser 稳定使用，而不是只生成一张好看的概念图。最新定位是私人金融助手，资产需要表达持仓监控、私人观察池、风险反驳、推荐决策、决策日志和长期记忆。
...
```text
pixel art fintech AI agent character concept,
memory archivist for private financial assistant,
small professional office character,
silver gray archive folder, tiny timeline badge,
dark office outfit with silver and soft gold accents,
calm meticulous posture, clear silhouette,
front view, side view, back view,
transparent background,
no text, no logo, no watermark
```
...
pixel art effect sprite sheet,
64x64 frame size, 8 frames,
watchlist board item softly glowing,
green and amber signal accents, small checklist card shape,
transparent background,
no readable text, no logo
```
...
pixel art effect sprite sheet,
64x64 frame size, 8 frames,
decision log archived into a small glowing file cabinet,
silver gray and soft gold particles, subtle timeline ribbon,
transparent background,
no readable text, no logo
```
...
7. 生成数据流、风险告警、审批通过、信号卡、观察池启动和记忆归档特效。
...
effects/
  ...
  fx_watchlist_trigger_v001.webp
  fx_memory_archive_v001.webp
```
