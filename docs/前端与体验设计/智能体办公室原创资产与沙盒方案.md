...
本文记录当前关于 `Agent 经营办公室` 原型的讨论结论、问题判断、原创资产路线和后续实现路径。最新产品定位是私人金融助手，因此办公室不只是“推荐分析动画”，还要表达持仓监控、私人观察池、提醒、长期记忆、决策日志和底层金融团队 Workflow。
...
- 上层私人金融主 Agent 决定调度哪个 Workflow，前端只展示公开状态事件，不展示隐藏推理链。
...
金融业务层
  持仓监控、观察池触发、行情刷新、信号计算、风控复核、换股比较、报告生成、订单草案、用户审批、复盘记忆
```
...
```json
{
  ...
  "agent": "coordinator",
  "targetZone": "meetingTable",
  "message": "观察池中的 600519 满足资金流转正条件，准备发起复核"
}
```
...
| 记忆管理员 | 安静、细致、负责复盘 | 银灰色档案夹、时间线徽章 | 归档决策、更新观察理由、安排复盘 |
...
portfolioWatch    持仓监控中
watchlistReview   观察池复核中
...
1 个任务卡资产
1 个风控警报特效
1 条流程：数据刷新 -> 持仓监控 -> 观察池触发 -> 风控复核 -> 决策日志写入 -> 下班待命
```
...
apps/agent-office/src/game/workflowEvents.ts
  映射后端 agent_workflow_events 到前端办公室状态
```
...
- 输出 `资产生成提示词.md`。
- 明确 1280×720 背景区域布局。
- 明确核心 Agent/角色的人设、服装、动作，并覆盖记忆管理员、观察池看板和持仓监控状态。
...
- 后端推送 `agent_workflow_events`、`monitoring_alerts`、`decision_logs` 的公开摘要。
- 前端只消费公开摘要和状态事件。
