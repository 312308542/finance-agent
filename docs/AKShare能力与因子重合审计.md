...
本文档用于回答一个核心问题：AKShare 已经提供了很多 A 股数据和部分技术筛选能力，系统在做因子计算、持仓监控和私人观察池管理前，应先判断哪些能力可以直接复用，哪些只能作为辅助标签，哪些必须由系统自己计算。

结论先行：

- AKShare 可以承担 A 股基础数据、A 股资金/财务/估值/风险/情绪数据，以及部分同花顺技术选股榜单。
- AKShare 不能替代本系统的统一因子计算层，因为系统需要同时覆盖 A 股和数字货币；两类市场分别推荐，但技术指标、缺失标记、可追溯快照和评分协议需要保持一致。
- AKShare 也不能替代上层私人金融主 Agent。持仓提醒、观察池升级/移除、换股建议、决策日志和 Finance Memory 必须由本系统基于结构化事实和用户上下文生成。
...
| 持仓提醒和观察池状态 | 使用 AKShare 输入 | 系统结合持仓、观察理由、触发条件和用户偏好判断 | 数据源只提供事实，不知道用户成本、仓位和历史决策 |
| Finance Memory 和复盘 | 不使用 AKShare | 系统写入 `decision_logs`、`assistant_memories`、`review_tasks` | 记忆来自建议、反馈和复盘，不来自单一数据接口，也不来自 Hermes 通用记忆 |
