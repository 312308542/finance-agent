...
| 私人金融助手 | `financial_memory_edges` | Finance Memory 的轻量图谱边表 | M2 |
...
3. 轻量关系图谱层：`financial_memory_edges`，第一阶段使用 PostgreSQL 边表表达关系，不引入独立图数据库。

观察池原因记忆不新增表，直接复用现有结构：`watchlist_items.reason` 保存观察项当前关注理由；`watchlist_item_events` 保存 `candidate_intake_reason`、`daily_watch_reason`、`watchlist_transition` 等可审计事件；`assistant_memories` 保存 `candidate_intake_reason` 和 `watchlist_daily_reason` 两类可召回摘要；`financial_memory_edges` 用 `watchlist_event -> assistant_memory` 的 `summarizes` 关系串起解释链路。

阶段性决策：第一版先用 PostgreSQL 表存图谱关系，避免在金融记忆模型仍在论证阶段就维护 Neo4j、Apache AGE、DozerDB 等额外中间件。等关系复杂度、路径推理、社区发现或跨资产传导分析成为刚需时，再从 `financial_memory_edges` 同步或迁移到专业图数据库。
...
这些事件可以通过 `source_decision_id` 关联 `decision_logs`，并通过 `financial_memory_edges` 的 `summarizes` 关系连接到对应的 `assistant_memories`。
...
Finance Memory 轻量图谱边表。第一阶段用关系数据库保存边，支持 SQL 查询一跳/两跳关系、审计链路和解释链路；后续如需专业图算法，再迁移到图数据库。
...
- 支持 Finance Memory 的向量召回和轻量图谱边表，第一阶段不引入独立图数据库。
- 支持订单草案。
- 保持推荐和真实下单分离。
