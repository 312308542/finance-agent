...
V1.2 触发事件表。它记录“为什么要唤起 Workflow”，不直接代表最终买卖建议。触发层只读取已入库的持仓、信号、TA 指标、因子、评分、风险、推荐运行和数据质量快照，不直接抓取行情，也不临时计算因子。
...
| `workflow_type` | string | 后续派发的 Workflow |
| `workflow_run_id` | string | 派发后的 Workflow Run ID，可为空 |
| `portfolio_id` | string | 组合 ID，可为空 |
...
- `idx_trigger_events_workflow`：`workflow_type, status`
