...
| `raw_records` | 原始数据响应 |
| `assets` | 资产稳定身份主表 |
| `asset_profiles` | 资产名称、行业、概念等慢变资料 |
| `asset_provider_mappings` | Provider 代码映射 |
| `asset_status_snapshots` | 交易状态快照 |
| `realtime_quote_snapshots` | 实时行情快照 |
| `asset_universes` | 候选池定义 |
| `asset_universe_members` | 候选池成员 |
| `market_bars` | 标准 OHLCV |
| `market_calendars` | 交易日历 |
| `fundamental_snapshots` | A 股财务估值快照 |
| `capital_flow_snapshots` | A 股资金流快照 |
| `crypto_derivative_snapshots` | 数字货币衍生品快照 |
| `event_records` | 新闻、公告和事件记录 |
| `evidence` | 推荐证据索引 |
| `positions` | 持仓快照 |
| `indicator_frames` | 指标计算结果 |
| `factor_frames` | 因子分组结果 |
| `screening_results` | 初筛结果 |
| `screening_result_items` | 初筛单标的明细 |
| `asset_scores` | 多维评分结果 |
| `signal_snapshots` | 信号快照 |
| `risk_findings` | 风险发现 |
| `recommendation_runs` | 推荐运行记录 |
| `recommendation_run_universes` | 推荐运行关联候选池 |
| `asset_recommendations` | 单标的推荐结果 |
| `agent_analysis_runs` | Agent 分析运行记录 |
| `agent_analysis_items` | Agent 单标的分析明细 |
