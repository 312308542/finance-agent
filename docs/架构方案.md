...
Finance Memory 存储层采用渐进式架构：

```text
PostgreSQL 结构化业务表
  -> decision_logs / assistant_memories / review_tasks / asset_theses
  -> memory_embeddings    # 向量召回，相似历史场景
  -> financial_memory_edges # 关系边表，轻量图谱
  -> 后续按需同步到专业图数据库
```

第一阶段不引入 Neo4j、Apache AGE、DozerDB 等独立图数据库。原因是当前首要目标是验证金融记忆能写、能查、能解释、能复盘；关系边表足以支持一跳/两跳审计和报告引用。等需要复杂路径推理、社区发现、跨资产传导关系或图算法时，再把 `financial_memory_edges` 作为迁移源。
...
- `financial_memory_edges` 只保存 `watchlist_event -> assistant_memory` 的 `summarizes` 关系，避免把观察池记忆过度设计成完整图数据库。
...
- `MemoryGraphService` 第一阶段只读写 PostgreSQL 边表 `financial_memory_edges`，不依赖独立图数据库。
