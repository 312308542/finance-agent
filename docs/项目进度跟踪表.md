...
| A 股数据 | P0/P1/P2 部分完成 | AKShare 能力注册表、行情 fallback、P1 Provider、P2 财务/估值 Provider、raw_records 归档已完成；部分东财接口当前网络断连 | 为行业/资金流/业绩报表补 curl_cffi fallback，继续接风险和情绪 Provider |
...
| P2 | 财报/业绩 | Provider 已完成，当前网络失败 | 已接 `stock_yjbb_em`、`stock_yjkb_em`、`stock_yjyg_em`；当前 `stock_yjbb_em` 请求东财 datacenter 超时，失败会写入 `raw_records` |
...
| 部分 P2 财报接口超时 | 业绩报表、快报、预告当前可能无法实时刷新 | 已记录错误到 `raw_records`，下一步补 curl_cffi fallback |
...
1. 为行业成分、资金流和业绩报表补 repo-side `curl_cffi` fallback。
