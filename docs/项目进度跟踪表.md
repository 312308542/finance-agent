...
| A 股数据 | P0/P1/P2 部分完成 | AKShare 能力注册表、行情 fallback、P1 Provider、P2 财务/估值 Provider、raw_records 归档已完成；资金流和业绩报表已有 fallback | 为行业/概念成分补非东财备用源，继续接风险和情绪 Provider |
...
| P1 | 行业/概念种子 | Provider 已完成，当前网络失败 | `stock_board_industry_cons_em` 当前 Eastmoney 主源和 curl_cffi fallback 都可能断连，下一步需补非东财成分源 |
...
| 表 | 当前验证结果 | 说明 |
| --- | --- | --- |
| `raw_records` | AKShare 已有 24 条 | 成功和失败响应均已归档，P1/P2 fallback 使用情况会写入 `actual_source` 和 `fallback_trace` |
| `event_records` | AKShare 新闻 3 条 | 来自 `stock_news_em` |
| `evidence` | AKShare 新闻证据 3 条 | 推荐解释可引用 |
| `capital_flow_snapshots` | 10 条 | 个股资金流 5 日榜已通过同花顺轻量 fallback 写入 |
...
| 问题 | 影响 | 当前处理 |
| --- | --- | --- |
| 部分东方财富板块成分接口被上游断连 | 行业/概念成分池当前可能拿不到真实数据 | 已记录错误到 `raw_records`，下一步补非东财板块成分源 |
...
1. 为行业/概念成分补非东财备用源。
2. 把 Binance/ccxt Provider 接入 `raw_records` 归档模式。
3. 建立统一采集命令，按 P0/P1/P2 分批刷新数据。
4. 接入 A 股风险和情绪 Provider，补停复牌、退市、质押、两融、龙虎榜、热度和涨停池。
5. 实现第一版指标和因子计算服务。
6. 实现初筛规则和多维评分。
