...
| 链路节点 | 状态 | 已有资产 | 缺口 |
| --- | --- | --- | --- |
| 股票池/币池 | 部分完成 | `assets`、`asset_universes`、`asset_universe_members` 表；A 股行业/概念种子 Provider；数字货币 markets Provider | 全 A、指数成分、热度池、资金流池、币种候选池刷新任务 |
| 数据采集 | 进行中 | AKShare P0 行情 fallback；AKShare P1 行业/资金流/新闻 Provider；AKShare P2 财务/估值 Provider；Binance 衍生品快照；`raw_records` 归档 | P2 风险/情绪 Provider；统一调度任务 |
| 原始响应审计 | 已完成基础版 | `RawRecordRepository`、`AshareP1Collector`、`AshareP2Collector` 已把成功和失败写入 `raw_records` | 后续所有 Provider 都要接入同一归档模式 |
...
| 表 | 当前验证结果 | 说明 |
| --- | --- | --- |
| `raw_records` | AKShare 已有 28 条 | 成功和失败响应均已归档，P1/P2 fallback 使用情况会写入 `actual_source`、`fallback_trace` 和必要的覆盖范围说明 |
...
1. 把 Binance/ccxt Provider 接入 `raw_records` 归档模式。
2. 建立统一采集命令，按 P0/P1/P2 分批刷新数据。
3. 接入 A 股风险和情绪 Provider，补停复牌、退市、质押、两融、龙虎榜、热度和涨停池。
4. 实现第一版指标和因子计算服务。
5. 实现初筛规则和多维评分。
6. 接入 Agent 分析、风险反驳和中文推荐报告。
