...
| T5 MCP 模板与握手冒烟 | 已完成 | 新增 `docs/集成/MCP接入模板.md`；`.\.venv\Scripts\python.exe -m pytest tests\test_hermes_mcp_handshake.py -q` 通过；`.\.venv\Scripts\python.exe scripts\integration\smoke_mcp_handshake.py` 通过，initialize/list_tools 成功，MCP wrapper 工具 20 个，内部事实工具 15 个 |
| T6 回归与文档同步 | 已完成 | 方案验收命令通过：`tests\test_hermes_tool_boundary.py` 3 passed；`smoke_hermes_skill_commands.py --mode direct` 通过；`smoke_mcp_handshake.py` 通过；补充 WSL 验收 `smoke_hermes_skill_commands.py --mode wsl-bridge` 通过；全量 `.\.venv\Scripts\python.exe -m pytest -q` 结果 `296 passed`；已同步 `00-总体规划与执行约定.md`、`docs/优化版本进度跟踪表.md` 和 `docs/项目进度跟踪表.md` |
