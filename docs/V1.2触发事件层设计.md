...
V1.2 的目标是让私人金融助手可以被已入库的数据变化唤起，而不是让 Workflow 常驻轮询；触发目标是 Hermes-Agent 或内部金融 Agent，不是直接运行 Workflow。触发层不直接抓取 AKShare、Binance、ccxt，也不临时计算 TA、因子、评分或信号；它只读取数据层已经清洗并写入 PostgreSQL + TimescaleDB 的事实。
...
flowchart LR
    ...
    D --> E["TriggerService.dispatch_pending\nAgent Wakeup Dispatcher"]
    E --> F["Hermes-Agent / 内部金融 Agent\n自由 loop / 工具调用 / 记忆上下文"]
    F --> G["按需调用金融团队 Workflow\n报告 / 提醒 / 记忆 / 审计"]
```

触发层只是“唤醒器”。是否买入、卖出、换股、入池或移除观察池，仍由被唤醒的上层 Agent 决策；Agent 可以按需调用底层金融团队 Workflow，并把结果写入审计链路。

## 2. 已支持触发类型

| 触发类型 | 数据来源 | 建议内部 Workflow | 说明 |
| --- | --- | --- | --- |
...
- `agent_runtime`：默认 `hermes_agent`，表示唤醒目标运行时。
- `agent_task_id`：派发后生成的 Agent 唤醒任务 ID。
- `requested_workflow_type`：建议 Agent 可按需调用的内部 Workflow，不是触发层直接运行目标。
...
- 观察池触发条件会使用 TA、因子和评分。
- 冷却期重复评估不会重复创建事件。
- 触发事件可以派发到 Agent 唤醒队列，并保留 `portfolio_monitoring`、`asset_deep_analysis` 和 `recommendation_decision` 作为建议的内部分析流程。
