...
本文档把 AKShare 官方股票数据能力映射到 Hermes 私人金融助手与 AI 标的推荐引擎。目标不是“把接口全部堆进来”，而是让每类数据都服务于完整推荐链路和持仓监控链路：
...
在最新两层 Agent 架构中，AKShare 既服务 A 股推荐，也服务：

- 持仓监控：价格波动、资金流、财报公告、停复牌和风险事件触发提醒。
- 私人观察池：热度、资金流、行业/概念、涨停池和事件数据用于发现值得持续观察的股票。
- 风险反驳：两融、龙虎榜、大宗交易、质押、退市、停牌等数据用于反方观点和失效条件。
- 复盘记忆：当观察项被加入、升级、移除或建议被采纳时，AKShare 证据需要进入决策日志和复盘。

更细的“AKShare 能力是否和系统因子计算重合、哪些应直接复用、哪些必须系统自算”的结论，见 [AKShare能力与因子重合审计](AKShare能力与因子重合审计.md)。
...
上层 `PersonalFinanceAgent` 也不直接调用 AKShare。它只能通过服务层读取已经落库的数据、观察池状态、提醒和证据，再决定是否调用底层金融团队 Workflow。
