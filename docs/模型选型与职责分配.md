...
本文记录私人金融助手当前阶段的模型分配决策。模型能力、价格、上下文长度、工具调用协议和供应商命名都可能变化，因此本文是“当前默认方案”，不是永久绑定。进入实现前，需要用实际 API 对结构化输出、工具调用、延迟、费用和稳定性做一次冒烟验证。
...
| 高风险最终复核 | GPT-5.5 Pro | 负责约 10% 的高风险复核，包括大仓位调整、卖出、换股、强冲突信号、数据缺口、连续亏损和重大风险事件 |
...
如果短期只接一个模型，优先使用 DeepSeek V4 Pro。它更适合作为长期运行的主 Agent：调用频率高、中文语境好、适合工具调用和结构化输出。GPT-5.5 Pro 更适合作为高质量、低频、关键决策复核模型，不适合每次行情波动都调用。
...
```mermaid
flowchart TD
    Trigger["实时/定时触发\n行情波动 / 持仓盈亏 / 观察池条件 / 用户请求"] --> Loop["DeepSeek V4 Pro\n主 Agent 决策循环"]
...
高风险复核不是另起一套决策系统，而是复查 DeepSeek V4 Pro 的初步建议、证据链、风险反驳和数据质量。GPT-5.5 Pro 的输出必须回写到同一套 `decision_logs`、`assistant_memories`、`review_tasks` 和 Workflow 审计链路中。
...
| 持仓大幅波动 | DeepSeek V4 Pro，命中高风险阈值后交给 GPT-5.5 Pro 复核 |
| 卖出候选 | DeepSeek V4 Pro + GPT-5.5 Pro |
| 换股候选 | DeepSeek V4 Pro + GPT-5.5 Pro |
| 大仓位调整 | DeepSeek V4 Pro + GPT-5.5 Pro |
| 数据缺口较多 | DeepSeek V4 Pro 先给低置信度建议；若仍涉及买卖或换股，交给 GPT-5.5 Pro 复核 |
| 成本敏感的低风险摘要 | 可选 Qwen3.5-Flash |
| 成本敏感的中文长报告 | 可选 Qwen3.5-Plus |
| 长周期策略复盘 / 私有化实验 | MiMo-V2.5-Pro 试验位 |

触发 GPT-5.5 Pro 升级复核的建议条件：
...
- GPT-5.5 Pro 是否能稳定读取 DeepSeek V4 Pro 的结构化初步建议、证据链和风险列表。
...
- GPT-5.5 Pro 的调用频率是否能被高风险阈值控制在低频范围内。
