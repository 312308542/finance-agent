...
Finance Memory 存储形式采用“结构化表 + 向量召回 + GraphStore 知识图谱”：

- 结构化表保存可审计事实：`decision_logs`、`assistant_memories`、`review_tasks`、`asset_theses`。
- 向量表保存语义召回索引：`memory_embeddings`，用于找相似历史场景和相似复盘教训。
- 图谱后端通过配置文件在 `neo4j` 和 `apache_age` 中二选一，同一环境只启用一个后端。
- 默认推荐 `neo4j`，使用 Neo4j / DozerDB 承载多跳查询、风险传导、社区发现和后续 GraphRAG。
- `apache_age` 是可选部署形态，适合希望减少独立中间件或资源较小的环境；它不是自动 fallback。
- 观察池原因不新增业务事实表：`watchlist_item_events` 记录 `candidate_intake_reason` 和 `daily_watch_reason`，`assistant_memories` 记录 `candidate_intake_reason` 和 `watchlist_daily_reason` 摘要，GraphSync 再投影出 `WatchlistEvent -> Memory` 等图谱关系。
- `financial_memory_edges` 作为历史兼容表短期保留，长期不再作为知识图谱主方案。
...
1. 后端先补私人金融助手数据库迁移和服务层：持仓、观察池、提醒、决策日志、Finance Memory、向量索引、GraphStore 知识图谱、复盘任务和 Workflow 审计。
