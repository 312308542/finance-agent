...
### 3.12 MemoryGraph：金融记忆知识图谱

`MemoryGraph` 表示 Finance Memory 的知识图谱层。它不是新的事实源，而是从 `decision_logs`、`assistant_memories`、`watchlist_item_events`、`review_tasks`、证据、风险、信号和因子摘要投影出的图谱视图。正式运行环境通过配置文件在 Neo4j / DozerDB 与 Apache AGE 中二选一，默认推荐 Neo4j / DozerDB；同一环境不双写双读，也不自动 fallback。

```json
{
  "graph_backend": "neo4j",
  "owner_id": "local_user",
  "source_table": "watchlist_item_events",
  "source_id": "watch_event:001",
  "source_node": "WatchlistEvent",
  "relation_type": "SUMMARIZED_BY",
  "target_node": "Memory",
  "target_source_id": "memory:watchlist_daily_reason:001",
  "confidence": 0.9
}
```

图谱关系类型由 GraphStore 统一语义定义，常用关系包括 `ABOUT`、`USES_EVIDENCE`、`WARNED_BY`、`GENERATES`、`SUMMARIZED_BY`、`SUPPORTS`、`CONTRADICTS`、`REVIEWS`、`ON_ASSET`、`HOLDS`、`WATCHES` 和 `AFFECTS`。`financial_memory_edges` 是历史兼容边表，短期保留以承接既有迁移和历史 smoke 数据；新增知识图谱能力应通过 GraphStore 写入配置选择的图数据库后端。
...
- Finance Memory 的知识图谱关系通过 GraphStore 承载，配置文件在 Neo4j / DozerDB 与 Apache AGE 中二选一；默认 Neo4j / DozerDB，Apache AGE 是另一种可选部署形态。
